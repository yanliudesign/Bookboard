...
export default function AddShelfModal({ onClose }: AddShelfModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      await apiRequest("POST", "/api/shelves", {
        name,
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold">New Shelf</h2>
          <button data-testid="button-close-shelf-modal" onClick={onClose} className="p-1">
            <X size={20} />
          </button>
        </div>
        ...
        <button
          ...
          className="w-full bg-black text-white py-3.5 rounded-full text-[16px] font-semibold disabled:opacity-40 transition-opacity"
        >
